...
public class GamePlay {
     ...
     * @throws InvalidUserActionException If an error occurs while placing flags.
     */
    public static void fillRoadWithFlags(GameMap map, Road road) throws InvalidUserActionException {
        if (road != null) {
    ...
    @Deprecated
    public static Road connectFlagToBuilding(Flag flag, Building building) throws Exception {
        System.out.println("Connecting " + flag + " to " + building);
        return connectPointToBuilding(flag.getPlayer(), flag.getPlayer().getMap(), flag.getPosition(), building);
    }

    public static void connectToPointByRoad(Point from, Point to, Player player, double roadPreference) throws InvalidUserActionException {
        var newRoads = placeRemainingRoads(findWayToConnectToBuilding(from, to, player, roadPreference), player);

        for (var road : newRoads) {
            fillRoadWithFlags(player.getMap(), road);
        }
    }

    public static void connectToBuildingByRoad(Point point, Building to, Player player, double roadPreference) throws Exception {
        var newRoads = placeRemainingRoads(findWayToConnectToBuilding(point, to.getPosition(), player, roadPreference), player);
    ...
    public static void connectToBuildingByRoad(Flag from, Building to) throws Exception {
        connectToBuildingByRoad(from.getPosition(), to, from.getPlayer(), 1.0);
    }
    ...
    public static Road connectPointToBuilding(Player player, GameMap map, Point start, Building building) throws Exception {
        System.out.println("Connecting " + start + " to " + building);

        var via = pointToConnectViaToGetToBuilding(player, map, start, building);
        System.out.println("via: " + via);

        return via != null
                ? map.placeAutoSelectedRoad(player, start, via)
                : map.placeAutoSelectedRoad(player, start, building.getFlag().getPosition());
     ...
     */
    @Deprecated
    private static Point pointToConnectViaToGetToBuilding(Player player, GameMap map, Point start, Building building) {
    ...
    public static List<Building> findVisibleOpponentBuildings(GameMap map, Player player) {
        var visibleLand = new HashSet<>(player.getDiscoveredLand());
    ...
    public static <T extends Building> T validate(T building) {
        if (building == null) {
            return null;
        }

        if (building.isDestroyed() || building.isBurningDown()) {
            return null;
        }
        ...
        if (!map.isBuildingAtPoint(point) || !Objects.equals(building, map.getBuildingAtPoint(point))) {
            return null;
        }
    ...
    public static int countTreesNearby(Point point, GameMap map, int radius) {
