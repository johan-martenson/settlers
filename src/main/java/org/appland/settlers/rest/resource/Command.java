package org.appland.settlers.rest.resource;

public enum Command {
    PLACE_FLAG,
    PLACE_ROAD,
    PLACE_BUILDING,
    REMOVE_ROAD,
    REMOVE_FLAG,
    REMOVE_BUILDING,
    CALL_SCOUT,
    CALL_GEOLOGIST,
    FULL_SYNC,
    INFORMATION_ON_POINTS,
    SET_RESERVED_IN_HEADQUARTERS,
    START_DETAILED_MONITORING,
    STOP_DETAILED_MONITORING,
    REMOVE_MESSAGE,
    SET_COAL_QUOTAS,
    GET_COAL_QUOTAS,
    GET_FOOD_QUOTAS,
    SET_FOOD_QUOTAS,
    GET_WHEAT_QUOTAS,
    SET_WHEAT_QUOTAS,
    GET_WATER_QUOTAS,
    SET_WATER_QUOTAS,
    GET_IRON_BAR_QUOTAS,
    SET_IRON_BAR_QUOTAS,
    PAUSE_GAME,
    RESUME_GAME,
    SET_STRENGTH_WHEN_POPULATING_MILITARY_BUILDING,
    GET_STRENGTH_WHEN_POPULATING_MILITARY_BUILDING,
    GET_DEFENSE_STRENGTH,
    SET_DEFENSE_STRENGTH,
    GET_DEFENSE_FROM_SURROUNDING_BUILDINGS,
    SET_DEFENSE_FROM_SURROUNDING_BUILDINGS,
    SET_GAME_SPEED,
    SET_MILITARY_POPULATION_FAR_FROM_BORDER,
    SET_MILITARY_POPULATION_CLOSE_TO_BORDER,
    SET_MILITARY_POPULATION_CLOSER_TO_BORDER,
    GET_POPULATE_MILITARY_CLOSE_TO_BORDER,
    GET_POPULATE_MILITARY_CLOSER_TO_BORDER,
    GET_POPULATE_MILITARY_FAR_FROM_BORDER,
    GET_SOLDIERS_AVAILABLE_FOR_ATTACK,
    SET_SOLDIERS_AVAILABLE_FOR_ATTACK,
    REMOVE_MESSAGES,
    GET_MILITARY_SETTINGS,
    UPGRADE,
    FLAG_DEBUG_INFORMATION,
    GET_GAME_INFORMATION,
    SET_GAME_NAME,
    SET_INITIAL_RESOURCES,
    SET_MAP,
    START_GAME,
    SET_OTHERS_CAN_JOIN,
    ADD_PLAYER_TO_GAME,
    REMOVE_PLAYER,
    UPDATE_PLAYER,
    CREATE_GAME,
    LISTEN_TO_GAME_LIST,
    STOP_LISTENING_TO_GAME_LIST,
    LISTEN_TO_GAME_INFO,
    STOP_LISTENING_TO_GAME_INFO,
    GET_MAPS,
    START_MONITORING_GAME,
    GET_GAMES,
    SET_GAME,
    SET_SELF_PLAYER,
    CREATE_PLAYER,
    SEND_CHAT_MESSAGE_TO_ROOM,
    LISTEN_TO_CHAT_MESSAGES,
    GET_CHAT_HISTORY_FOR_ROOM,
    ATTACK_HOUSE,
    EVACUATE_HOUSE,
    FIND_NEW_ROAD,
    DELETE_GAME,
    PAUSE_PRODUCTION,
    RESUME_PRODUCTION,
    DISABLE_PROMOTIONS,
    ENABLE_PROMOTIONS,
    CANCEL_EVACUATION,
    SET_TRANSPORT_PRIORITY,
    GET_TERRAIN,
    GET_TRANSPORT_PRIORITY,
    PLACE_FLAG_AND_ROAD,
    LISTEN_TO_STATISTICS,
    STOP_LISTENING_TO_STATISTICS,
    MARK_GAME_MESSAGES_READ,
    GET_TOOL_PRODUCTION_PRIORITIES,
    SET_TOOL_PRODUCTION_PRIORITY,
    GET_MAP,
    CHEAT,
    SET_CHEATING_ON_OFF,
    CLEAR_GAME,
    LISTEN_TO_PLAYER, STOP_LISTENING_TO_PLAYER, BLOCK_MATERIAL, ALLOW_MATERIAL, SEND_OUT_MATERIAL, STOP_SENDING_OUT_MATERIAL, GET_MAPS_WITH_TERRAIN, GET_MAP_WITH_TERRAIN, GET_STATISTICS
}
