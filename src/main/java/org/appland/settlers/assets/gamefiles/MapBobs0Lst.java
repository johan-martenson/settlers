package org.appland.settlers.assets.gamefiles;

public class MapBobs0Lst {
    public static final String FILENAME = "DATA/MAPBOBS0.LST";

    public static final int MINI_BROWN_MUSHROOM = 283;
    public static final int BROWN_MUSHROOM = 329;
    public static final int MINI_STONE_WITH_GRASS = 330;
    public static final int SMALL_STONE_WITH_GRASS = 331;
    public static final int SOME_SMALL_STONES = 332;
    public static final int SOME_SMALLER_STONES = 333;
    public static final int SPARSE_BUSH = 335;
    public static final int FEW_SMALL_STONES = 334;
    public static final int SOME_WATER = 336;
    public static final int LITTLE_GRASS = 337;
    public static final int SNOWMAN = 338;
    public static final int PORTAL = 339;
    public static final int SHINING_PORTAL = 340;
    public static final int PORTAL_SHADOW = 397;
    public static final int SHINING_PORTAL_SHADOW = 398;
    public static final int MINI_BROWN_MUSHROOM_SHADOW = 341; // 283 + 58
    public static final int BROWN_MUSHROOM_SHADOW = 387;
    public static final int DONKEY_EAST_ANIMATION = 785;
    public static final int DONKEY_SOUTH_EAST_ANIMATION = 793;
    public static final int DONKEY_SOUTH_WEST_ANIMATION = 801;
    public static final int DONKEY_WEST_ANIMATION = 809;
    public static final int DONKEY_NORTH_WEST_ANIMATION = 817;
    public static final int DONKEY_NORTH_EAST_ANIMATION = 825;
    public static final int DONKEY_EAST_SHADOW = 833;
    public static final int DONKEY_SOUTH_EAST_SHADOW = 834;
    public static final int DONKEY_SOUTH_WEST_SHADOW = 835;
    public static final int MINI_STONE_WITH_GRASS_SHADOW = 388;
    public static final int SMALL_STONE_WITH_GRASS_SHADOW = 389;
    public static final int SOME_SMALL_STONES_SHADOW = 390;
    public static final int SOME_SMALLER_STONES_SHADOW = 391;
    public static final int FEW_SMALL_STONES_SHADOW = 392;
    public static final int SPARSE_BUSH_SHADOW = 393;
    public static final int SOME_WATER_SHADOW = 394;
    public static final int LITTLE_GRASS_SHADOW = 395;
    public static final int SNOWMAN_SHADOW = 396;
}
